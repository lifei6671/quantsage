...
开发服务器监听 `127.0.0.1:4173`，并通过 Vite proxy 将 `/api/*` 转发到 `http://127.0.0.1:8080`。浏览器实际请求同源 `http://127.0.0.1:4173/api/*`，因此开发阶段不会触发跨域，也可以正常携带 HttpOnly session cookie。

如果后端不是默认地址，可以只改代理目标：

```bash
VITE_DEV_API_PROXY_TARGET=http://127.0.0.1:8090 npm run dev
```
...
`preview` 同样会把 `/api/*` 转发到 `VITE_DEV_API_PROXY_TARGET` 指向的后端，未设置时默认 `http://127.0.0.1:8080`，方便本地验证构建产物。
